...
$text_field_data_attr = (string) $this->getConfig('textfield_data_attr');
$def_field_data_attr = (string) $this->getConfig('deffield_data_attr');
$content  = '';
$message  = '';
...
$applyDataAttributes = static function ($field, string $attributeString): void {
  if ('' === trim($attributeString)) {
    return;
  }

  $matches = [];
  preg_match_all('/\b(data-[a-z0-9:\-]+)\s*=\s*(?:"([^"]*)"|\'([^\']*)\'|([^\s"\']+))/i', $attributeString, $matches, PREG_SET_ORDER);
...
if ($func == '') {
  ...
  $applyDataAttributes($field, $def_field_data_attr);
  ...
  $descriptionClass = trim((string) $editor);
  if ('' === $descriptionClass) {
  ...
  $applyDataAttributes($field, $text_field_data_attr);
